...
class FEKFMBL(GFLocalization, MapFeature):
    ...
    def h(self, xk):  # overloaded to stack measurements and feature observations
        ...
        * Feature Observations: :math:`z_f` correspond to observations of the features (CartesianFeature, PolarFeature, EsphericalFeature, etc...).
        ...
        if self.measurements == False:
            h_mf = self.hf(xk)
        elif self.features == False:
            h_mf = self.hm(xk)
        else:
            h_mf = np.block([[self.hm(xk)],[self.hf(xk)]])
    ...
    def SquaredMahalanobisDistance(self, hfj, Pfj, zfi, Rfi):
        ...
        nu_ij = zfi - hfj
        S_ij = Rfi + Pfj
        D2_ij = nu_ij.T @ np.linalg.inv(S_ij) @ nu_ij
    ...
    def SplitFeatures(self, zf, Rf, H):
        ...
        for i in range(len(H)):
            Fj = H[i]
            if Fj != None:
                zp = np.block([[zp],[zf[2*i:2*i+2]]]) # bcs 1 obs is represented by 2 rows in zf
                Rp = scipy.linalg.block_diag(Rp,Rf[2*i:2*i+2,2*i:2*i+2]) # bcs 1 obs is represented by 2x2 matrix in Rf
                Hp = np.block([[Hp],[self.Jhfjx(self.xk_bar,Fj)]]) # not sure
                Vp = scipy.linalg.block_diag(Vp,np.eye(self.xF_dim))
            else:
                znp = np.block([[znp],[zf[2*i:2*i+2]]])
                Rnp = scipy.linalg.block_diag(Rnp,Rf[2*i:2*i+2,2*i:2*i+2])
