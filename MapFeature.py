...
class MapFeature:
    ...
    * **The storage representation:** which is the representation used to store the feature in the map within the state vector.
    ...
    def hf(self, xk):  # Observation function for al zf observations
        ...
        n = self.H.shape[0] # number of feature

        for i in range(n):
            Fj = self.H[i] # H = hypothesis

            if not (Fj == None):
                h = np.block([[h], [self.hfj(xk,Fj)]])    
    ...
    #     Hk = np.array([]) # not sure the best way to initialize this
    #     n = self.robot.M.shape[0]

    #     for i in range(n):
    #         Fj = self.robot.M[i] # not sure
    #         Hk = np.block([[Hk],[self.Jhfjx(xk,Fj)]])
    ...
    def hfj(self, xk_bar, Fj):  # Observation function for zf_i and x_Fj
        ...
        # TODO: To be implemented by the student
        NxB = Pose3D(xk_bar[0:3]) # taking only the first 3 rows to ensure that we're only taking the pose
        NxF = self.robot.M[int(Fj)]

        return self.s2o(NxB.ominus().boxplus(NxF))
    ...
    def Jhfjx(self, xk, Fj):  # Jacobian wrt x of the observation function for feature observation i
        ...
        # J_hfjx = J_s2o @ J_1[+] * J_(-)
        # TODO: To be implemented by the student
        NxB = Pose3D(xk[0:3]) # taking only the first 3 rows to ensure that we're only taking the pose
        NxF = self.robot.M[int(Fj)]

        Jp = self.J_s2o(NxB.ominus().boxplus(NxF)) @ NxB.ominus().J_1boxplus(NxF) @ NxB.J_ominus()
        Jnp = np.zeros((Jp.shape[0],xk.shape[0]-3))

        return np.block([Jp, Jnp])
...
class Cartesian2DMapFeature(MapFeature):
    ...
    def GetFeatures(self):
        ...
        """
        # TODO: To be implemented by the student
        (zk,Rk) = self.robot.ReadCartesianFeature()
