...
class FEKFSLAMFeature(MapFeature):
    ...
    def hfj(self, xk_bar, Fj):  # Observation function for zf_i and x_Fj
        ...
        NxF = CartesianFeature(xk_bar[2*Fj+3:2*Fj+5])
    ...
    def Jhfjx(self, xk, Fj):  # Observation function for zf_i and x_Fj
        ...
        NxF = CartesianFeature(xk[2*Fj+3:2*Fj+5])

        # Jacobian related to robot pose
        Jp = self.J_s2o(NxB.ominus().boxplus(NxF)) @ NxB.ominus().J_1boxplus(NxF) @ NxB.J_ominus()

        # Jacobian related to features in the state
        Jf = np.zeros((Jp.shape[0],xk.shape[0]-3))
        Jf[:,2*Fj:2*Fj+2] = self.J_s2o(NxB.ominus().boxplus(NxF)) @ NxB.ominus().J_2boxplus(NxF)

        # J = Jhr ...0 Jhfi ...0
        return np.block([Jp, Jf])
