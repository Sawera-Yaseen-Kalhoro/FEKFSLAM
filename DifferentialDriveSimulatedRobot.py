...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def __init__(self, xs0, map=[],*args):
        ...
        * **Rfp** : Object attribute containing the covariance of the simulated Polar2D feature noise (:math:`R_{fp}=diag(\\sigma_{\\rho}^2,\\sigma_{\\phi}^2)`)
        ...
        self.Rfc = np.diag(np.array([0.5 ** 2, 0.5 ** 2])) # covariance of simulated cartesian feature noise -- ADDED
        ...
        self.yaw_reading_frequency = 5  # frequency of Yaw readings
    ...
    def fs(self, xsk_1, usk):  # input velocity motion model with velocity noise
        ...
        **To be completed by the student**.
        ...
        etask_1 = Pose3D(xsk_1[0:3]) # previous pose
        nusk_1 = Pose3D(xsk_1[3:6]) # previous velocity
        nu_d = np.array([[usk[0,0]],[0.0],[usk[1,0]]]) # desired velocity

        wsk = np.reshape(np.random.multivariate_normal([0.0,0.0,0.0], self.Qsk), (3,1)) # sample Gaussian for the acceleration perturbation
        
        K = np.diag([1,1,1]) # velocity gain

        etask = etask_1.oplus(nusk_1*self.dt + 1/2*wsk*self.dt**2)
        nusk = nusk_1 + np.matmul(K,(nu_d - nusk_1)) + wsk*self.dt

        self.xsk = np.row_stack((etask, nusk))
    ...
    def ReadEncoders(self):
        ...
        **To be completed by the student**.
        ...
        # get variables from the class attributes
        vr = self.xsk[3,0]
        wr = self.xsk[5,0]
        
        # linear velocity of each wheel
        vL = vr - wr*self.wheelBase/2
        vR = vr + wr*self.wheelBase/2

        # linear displacement
        v = np.array([[vL],[vR]])
        d = v*self.dt

        # convert linear displacement to pulse
        self.zsk = d / (2*np.pi*self.wheelRadius) * self.pulse_x_wheelTurns

        # compute Rsk = noise
        Rsk = np.reshape(np.random.multivariate_normal([0.0,0.0], self.Re), (2,1))

        return self.zsk + Rsk , self.Re
    ...
    def ReadCompass(self):
        ...
        # compute noise
        R_yaw = np.random.normal(loc=0, scale=self.v_yaw_std)

        # returns value only within the frequency
        if self.k % self.yaw_reading_frequency == 0:
            # self.yaw_reading = True
            return self.xsk[2,0] + R_yaw, self.v_yaw_std
        else:
            # self.yaw_reading = False
            return None,None
    ...
    def ReadRanges(self):
        etak = self.xsk[0:2] # position of the robot

        # position of the landmarks
        landmarks_pos = self.M
        ...
        for landmark in landmarks_pos:
            ...
            d = np.sqrt((etak[0]-landmark[0])**2 + (etak[1]-landmark[1])**2)
            # compute noise
    ...
    def ReadCartesianFeature(self):
        """
            Returns the cartesian coordinates of the features observed in the robot frame.
            The observation is limited by the max range.
        """
        xsk = self.xsk[0:3] # taking only the first 3 rows to ensure that we're only taking the pose

        # initialize variables for stacking
        zf = np.zeros((0, 1))  # empty vector
        Rf = np.zeros((0, 0))  # empty matrix

        for landmark in self.M:
            # calculate distance using Euclidean distance
            d = np.sqrt((xsk[0]-landmark[0])**2 + (xsk[1]-landmark[1])**2)
            # set the condition based on max range
            if d <= self.xy_max_range:
                Bvk = np.reshape(np.random.multivariate_normal([0.0,0.0], self.Rfc), (2,1))
                BxF = Pose3D(xsk).ominus().boxplus(landmark) + Bvk

                # stacking
                zf, Rf = np.block([[zf], [BxF]]), scipy.linalg.block_diag(Rf, self.Rfc)

        # returns value only within the frequency
        if self.k % self.xy_feature_reading_frequency == 0:
            # self.feature_reading = True
            return zf,Rf
        else:
            # self.feature_reading = False
            return None,None
